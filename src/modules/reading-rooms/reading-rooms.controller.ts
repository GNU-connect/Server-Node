...
export class ReadingRoomsController {
  ...
  public async listReadingRooms(@Body() body: SkillPayloadDto): Promise<ResponseDTO> {
    const { campusId } = plainToInstance(ListReadingRoomsRequestDto, body.action.clientExtra);
  ...
  public async getReadingRoomDetail(@Body() body: SkillPayloadDto): Promise<ResponseDTO> {
    const { readingRoomId } = plainToInstance(GetReadingRoomDetailRequestDto, body.action.clientExtra);
    const template = await this.readingRoomsService.readingRoomDetailComplexCard(
      readingRoomId,
    );
    return new ResponseDTO(template);
  }
