...
## 8. 남은 확인 사항(구현 계획 단계에서 처리)

- Python `cafeteria_repository.py`가 `cafeteria.last_date`를 갱신하는지 등 저장 세부 동작을 확인해 맞춘다.
- 학사일정 POST가 세션·쿠키 없이 계속 동작하는지 fixture와 함께 재확인하고, 사이트가 바뀌어도 0건 가드가 실패로 잡아내는지 확인한다.
