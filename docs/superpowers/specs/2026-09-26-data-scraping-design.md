...
- 마이그레이션 적용 후 새 코드 배포 전까지 구버전 배치가 `notice` 타입 run을 만들면 체크 제약에 걸려 실패한다. 이때 `scrape_run` 행은 만들어지지 않고 배치 로그에만 남는다(기록되지 않는다). 매시 cron이라 영향은 한 사이클이고, 구 `NoticeJob`은 아무 일도 하지 않는 골격이라 데이터 손실은 없다.
- 마이그레이션은 배포 시점에 대기/실행 중이던 옛 `notice` run을 먼저 `failed`로 바꾼 뒤 타입 이름을 바꾼다(대상이 없는 채 활성 상태로 남아 새 배치를 실패시키지 않도록).
- 마이그레이션과 API·배치·어드민 웹 배포는 같은 릴리스에서 진행한다.
